...
const App = {
  ...
  checkValidKeystore: function (keystore) {
    ...
    const isValidKeystore =
      parsedKeystore.version && parsedKeystore.id && parsedKeystore.address && parsedKeystore.keyring;
  ...
  mintYTT: async function (videoId, author, dateCreated, hash) {
    ...
    const feePayer = cav.klay.accounts.wallet.add(PRIVATE_KEY);

    const estimatedGas = await yttContract.methods
      .mintYTT(videoId, author, dateCreated, 'https://ipfs.infura.io/ipfs/' + hash)
  ...
  approve: async function () {
    ...
    const estimatedGas = await yttContract.methods
      ...
      .estimateGas({from: walletInstance.address});
  ...
  checkApproval: async function (walletInstance) {
    var isApproval = await this.isApprovedForAll(walletInstance.address, DEPLOYED_ADDRESS_TOKENSALES);
    if (isApproval) {
      $('#approve').hide();
    } else {
      $('#cancelApproval').hide();
  ...
  buyToken: async function (button) {
    ...
    try {
      ...
      const estimatedGas = await tsContract.methods
        ...
        .estimateGas({from: sender.address, value: price});
